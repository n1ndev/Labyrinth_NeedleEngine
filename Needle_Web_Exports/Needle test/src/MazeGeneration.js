...
var middleWalls = new Array(10).fill(true);
var innerBarrier = new Array(10).fill(true);
var outerBarrier = new Array(10).fill(true);
var stack = [];
GenerateMaze();
...
function GetLastIndexAtLayer(layer) {
    ...
    switch (direction) {
        case 'outward':
            {
                if (position.longitude == GetLastLayerIndex()) {
                    if (debug) {
                        console.log("No position outward of ".concat(position.longitude, ", ").concat(position.latitude));
                    }
                    return null;
                }
                else {
        ...
        case 'inward':
            {
                if ((position.longitude - 1) == 0) {
                    if (debug) {
                        console.log("No position inward of ".concat(position.longitude, ", ").concat(position.latitude));
                    }
                    return null;
                }
                else {
        ...
        case 'clockwise': {
            if (position.latitude == GetLastIndexAtLayer(position.longitude)) {
                ...
                return { longitude: position.longitude, latitude: position.latitude + 1 };
            break;
        }
        case 'anticlockwise': {
...
function GetUnvisitedNeighbours(position) {
    ...
    // {
    // }
    var adjacentPosition = GetAdjacentPosition(position, 'outward');
...
function GenerateMaze() {
    ...
    while (stack.length > 0) {
        console.log("Generation Step: ".concat(i));
        ...
        if (neighbours.length > 0) {
            stack.push(currentCell);
            var chosenCell = neighbours[rnd.randomInRange(0, neighbours.length - 1)];
...
function RemoveBarrier(currentPos, movement) {
    // We need to get the corresponding barrier and set to false!
    switch (movement.direction) {
        case 'outward':
            {
                if (debug)
                    console.log("Removing middle wall at ".concat(currentPos.latitude));
        ...
        case 'clockwise':
            {
                if (currentPos.latitude == GetLastIndexAtLayer(currentPos.longitude)) {
                    if (currentPos.longitude == 2) {
                        if (debug)
                            console.log("Removing outer barrier: 0");
                        outerBarrier[0] = false;
                    }
                    else if (currentPos.longitude == 1) {
                        if (debug)
                            console.log("Removing inner barrier: 0");
                    ...
                    if (currentPos.longitude == 2) {
                        ...
                        if (debug)
                            console.log("Removing outer barrier: ".concat(movement.position.latitude));
                    }
                    else if (currentPos.longitude == 1) {
                        if (debug)
                            console.log("Removing inner barrier: ".concat(movement.position.latitude));
        ...
        case 'anticlockwise':
            {
                if (currentPos.latitude == 0) {
                    if (currentPos.longitude == 2) {
                        outerBarrier[0] = false;
                        if (debug)
                            console.log("Removing outer barrier: ".concat(GetLastIndexAtLayer(2)));
                    }
                    else if (currentPos.longitude == 1) {
                        innerBarrier[0] = false;
                        if (debug)
                            console.log("Removing inner barrier: ".concat(GetLastIndexAtLayer(2)));
                    }
                }
                else {
                    if (currentPos.longitude == 2) {
